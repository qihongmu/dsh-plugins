...
Token Tracing is not on npm yet — install it from a built source checkout of this repository (see [CONTRIBUTING.md](../../CONTRIBUTING.md)); the repository root README carries the plugin ↔ dsh compatibility table (verified against `dsh-v0.1.2-rc.1`).

```sh
dsh plugin --profile web add ./packages/token-tracing/host ./packages/token-tracing/remotes ./packages/token-tracing/client
```

One command mounts all three halves (host service, remotes assembly, browser UI) into the web profile. Restart `dsh web` afterwards. Conversations that predate the install are backfilled automatically in the background — no waiting for data to accumulate.
