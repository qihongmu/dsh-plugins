`@qihongmu/dsh-client-remotes-token-tracing` is the **remotes assembly half** of the Token Tracing plugin — it mounts the vendored `tokenTracing` Remote contribution before the browser UI activates.
