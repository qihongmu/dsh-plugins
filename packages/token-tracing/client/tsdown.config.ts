/**
 * Standalone browser-bundle config for the external token-tracing UI plugin.
 *
 * Mirrors the scheduled-task config: the dsh repo's `clientBundle` preset
 * hard-pins its workspace manifest to the dsh checkout, so this config
 * reproduces only what this package needs — a CJS browser bundle at
 * lib/client.js, `react` and the shared UI primitives as module-table
 * externals, and `.module.css` compiled through lightningcss with a virtual
 * self-injecting class-map module.
 *
...
const PLUGIN_ID = '@qihongmu/dsh-client-ui-token-tracing'
...
export default defineConfig({
  name: '@qihongmu/dsh-client-ui-token-tracing/client',
